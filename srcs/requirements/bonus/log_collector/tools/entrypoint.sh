...
COLLECTION_INTERVAL=${LOG_COLLECTION_INTERVAL:-60}
...
while true; do
    SERVICES=$(curl -s --unix-socket /var/run/docker.sock "http://localhost/v1.41/services" | grep -o '"Name":"[^"]*' | cut -d'"' -f4 | grep "^${STACK_NAME}")
    ...
    for SERVICE_FULL_NAME in $SERVICES; do
        ...
        curl -s --unix-socket /var/run/docker.sock \
            "http://localhost/v1.41/services/${SERVICE_FULL_NAME}/logs?stdout=true&stderr=true&timestamps=true&since=${TIME_DIFF}" | \
            sed 's/^.\{8\}//' | \
